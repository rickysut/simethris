...
class Tag extends CI_Controller {
	public function detail(){
		...
		if ($query->num_rows()<=0){
			...
			$data['rows'] = $row;
			if (is_numeric($dari)) {
				$data['beritatag'] = $this->db->query("SELECT berita.*, users_web.nama_lengkap, kategori.nama_kategori, kategori.kategori_seo FROM berita 
															left join users_web on berita.username=users_web.username
