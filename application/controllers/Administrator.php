...
class Administrator extends CI_Controller {
    ...
    function reset_password(){
        if (isset($_POST['submit'])){
            $usr = $this->model_app->edit('users_web', array('id_session' => $this->input->post('id_session')));
            if ($usr->num_rows()>=1){
                if ($this->input->post('a')==$this->input->post('b')){
                    ...
                    $this->model_app->update('users_web', $data, $where);
    ...
    function lupapassword(){
        if (isset($_POST['lupa'])){
            ...
            $cekemail = $this->model_app->edit('users_web', array('email' => $email))->num_rows();
            if ($cekemail <= 0){
                $data['title'] = 'Alamat email tidak ditemukan';
                $this->load->view('administrator/view_login',$data);
            }else{
                $iden = $this->model_app->edit('identitas', array('id_identitas' => 1))->row_array();
                $usr = $this->model_app->edit('users_web', array('email' => $email))->row_array();
	...
	function home(){
        if ($this->session->level=='admin'){
          ...
          $data['users'] = $this->model_app->view_where('users_web',array('username'=>$this->session->username))->row_array();
          $data['modul'] = $this->model_app->view_join_one('users_web','users_modul','id_session','id_umod','DESC');
	...
	function menuwebsite(){
		...
		$data['record'] = $this->model_app->view_ordering('menuweb','urutan','ASC');
	...
	function tambah_menuwebsite(){
		...
		if (isset($_POST['submit'])){
			...
			$this->model_app->insert('menuweb',$data);
			redirect('administrator/menuwebsite');
		}else{
			$proses = $this->model_app->view_where_ordering('menuweb', array('position' => 'Bottom'), 'id_menu','DESC');
			$data = array('record' => $proses);
	...
	function edit_menuwebsite(){
		...
		if (isset($_POST['submit'])){
			...
			$this->model_app->update('menuweb', $data, $where);
			redirect('administrator/menuwebsite');
		}else{
			$menu_utama = $this->model_app->view_where_ordering('menuweb', array('position' => 'Bottom'), 'id_menu','DESC');
			$proses = $this->model_app->edit('menuweb', array('id_menu' => $id))->row_array();
	...
	function listberita(){
        ...
        $data['rss'] = $this->model_utama->view_joinn('berita','users_web','kategori','username','id_kategori','id_berita','DESC',0,10);
	...
	function manajemenuser(){
		...
		$data['record'] = $this->model_app->view_ordering('users_web','username','DESC');
	...
	function tambah_manajemenuser(){
		...
		if (isset($_POST['submit'])){
            ...
            $this->model_app->insert('users_web',$data);
	...
	function edit_manajemenuser(){
		...
		if (isset($_POST['submit'])){
            ...
            if ($this->session->username==$this->uri->segment(3) OR $this->session->level=='admin'){
                $proses = $this->model_app->edit('users_web', array('username' => $id))->row_array();
	...
	function delete_manajemenuser(){
        ...
        $this->model_app->delete('users_web',$id);
