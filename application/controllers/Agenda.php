...
class Agenda extends CI_Controller {
	public function index(){
			...
			if (is_numeric($dari)) {
				$data['agenda'] = $this->model_utama->view_join('agenda','users_web','username','id_agenda','DESC',$dari,$config['per_page']);
			}else{
				redirect('main');
			}
	...
	public function detail(){
		$query = $this->model_utama->view_join_one('agenda','users_web','username',array('tema_seo' => $this->uri->segment(3)),'id_agenda','DESC',0,1);
		if ($query->num_rows()<=0){
			redirect('main');
		}else{
			$row = $query->row_array();
