...
class Berita extends CI_Controller {
	public function index(){
			...
			if (is_numeric($dari)) {
				if ($this->input->post('kata')<>''){
					...
					$data['description'] = description();
					$data['keywords'] = keywords();
					$data['berita'] = $this->model_utama->view_joinn('berita','users_web','kategori','username','id_kategori','id_berita','DESC',$dari,$config['per_page']);
					$this->pagination->initialize($config);
				}
			}else{
				redirect('main');
	...
	public function detail(){
		$query = $this->model_utama->view_join_two('berita','users_web','kategori','username','id_kategori',array('judul_seo' => $this->uri->segment(3)),'id_berita','DESC',0,1);
