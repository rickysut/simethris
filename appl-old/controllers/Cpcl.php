...
class Cpcl extends CI_Controller{
  ...
  function list_cpcl($idriph)
  {
    ...
    $this->data['get_all_cpcl_by_iduser'] = $this->Cpcl_model->get_all_cpcl_by_iduser($idriph);
	$this->data['get_all_combobox_provinsi'] = $this->m_wilayah->get_all_provinsi();
    $this->load->view('back/cpcl/list_cpcl', $this->data);
  }
  ...
  function create($idriph)
  {
	...
	$this->data['pola_bagi_hasil'] = [
      'name'          => 'pola_bagi_hasil',
      'id'            => 'pola_bagi_hasil',
      'class'         => 'form-control form-control-sm',
    ];
  ...
  function aju_verifikasi_action()
  {
	...
	public function ajax_list($idpoktan)
	{
		...
		foreach ($list as $person) {
			...
			//add html for action
			$row[] = '<a class="btn btn-sm btn-primary btn-fab" href="javascript:void(0)" title="Isi Data Rencana" onclick="edit_person('."'".$person->id_petani."'".')"><i class="icon icon-account-card-details"></i></a>
				  <a class="btn btn-sm btn-primary btn-fab" href="javascript:void(0)" title="Isi Posisi" onclick="edit_personPeta('."'".$person->id_petani."','".$lat."','".$lng."'".')"><i class="icon icon-google-maps"></i></a>
				  <a class="btn btn-sm btn-danger btn-fab">hapus</a>';
	...
	public function ajax_list_peta($idcpcl)
	{
		...
		if(!empty($list)){
			foreach ($list as $lokasi) {
				...
				$row = array();
				$row[] = $no;
				$row[] = $lokasi->id_lokasi;
				$row[] = $lokasi->id_petani;
				$row[] = $lokasi->nik;
				$row[] = $lokasi->nama_petani;
				$row[] = $lokasi->rencana_luas_tanam;
				$row[] = $lokasi->atitude;
				$row[] = $lokasi->latitude;
				$row[] = $lokasi->file_kml;

				//add html for action
				$row[] = '<a class="btn btn-sm btn-primary btn-fab" href="javascript:void(0)" title="Isi Data Rencana" onclick="edit_person('."'".$lokasi->id_lokasi."'".')"><i class="icon icon-account-card-details"></i></a>
						<a class="btn btn-sm btn-primary btn-fab" href="javascript:void(0)" title="Isi Posisi" onclick="edit_personPeta('."'".$lokasi->id_lokasi."','".$lat."','".$lng."'".')"><i class="icon icon-google-maps"></i></a>
						<a class="btn btn-sm btn-danger btn-fab">hapus</a>';
						//<a class="btn btn-sm btn-danger btn-fab" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$person->id_petani."'".')"><i class="icon icon-delete-empty"></i></a>';
	...
	public function ajax_list_saprodi($id)
	{
		...
		foreach ($list as $saprodi) {
			...
			//add html for action
			$row[] = '<a class="btn btn-sm btn-primary btn-fab" href="javascript:void(0)" title="Isi Posisi" onclick="edit_saprodi('."'".$saprodi->id_saprodi."'".')"><i class="icon icon-lead-pencil"></i></a>
				  <a class="btn btn-sm btn-danger btn-fab" href="javascript:void(0)" title="Hapus" onclick="delete_saprodi('."'".$saprodi->id_saprodi."'".')"><i class="icon icon-delete-empty"></i></a>';
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->saprodi->count_all(),
						"recordsFiltered" => $this->saprodi->count_filtered(),
						"data" => $data,
				);
		//output to json format
		echo json_encode($output);
	...
	public function ajax_add2()
	{
		...
		$namaPoktan="";$idPetani = "";$namaPetani = "";$nikPetani = "";$ProvPetani="";$KabPetani = "";$KecPetani = "";$DesPetani = "";$AlmPetani = "";
		if(!empty($petani)){
			foreach($petani as $dataPetani){
				$idPetani = $dataPetani->id_petani;
