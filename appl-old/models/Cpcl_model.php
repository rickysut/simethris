...
class Cpcl_model extends CI_Model{
  ...
  {
    $this->db->select('cpcl.id_cpcl, cpcl.no_pks, cpcl.tgl_rencana_tanam, cpcl.rencana_produksi, cpcl.tgl_awal_perjanjian, cpcl.tgl_akhir_perjanjian, cpcl.nama_pelaksana, cpcl.is_active, cpcl.is_verifikasi, if(cpcl.type_pelaksana=2, "Swakelola", "Kemitraan") as namapengelola, 
  ...
  {
    $this->db->select('cpcl.id_cpcl, cpcl.id_mst_riph, cpcl.no_pks, cpcl.tgl_rencana_tanam, cpcl.rencana_produksi, cpcl.tgl_awal_perjanjian, cpcl.tgl_akhir_perjanjian, cpcl.nama_pelaksana, cpcl.is_active, cpcl.is_verifikasi, cpcl.is_aju_verifikasi, if(cpcl.type_pelaksana=2, "Swakelola", "Kemitraan") as namapengelola, 
	...
	from realisasi where realisasi.is_delete=0 group by realisasi.id_users, realisasi.id_cpcl) t1', 't1.id_users = cpcl.id_users and t1.id_cpcl = cpcl.id_cpcl','left');
	$this->db->join('(select produksi.id_users AS id_users,produksi.id_cpcl AS id_cpcl,sum(produksi.jml_produksi) AS jmlproduksi 
	from produksi where produksi.is_delete=0 group by produksi.id_users, produksi.id_cpcl) t2', 't2.id_users = cpcl.id_users and t2.id_cpcl = cpcl.id_cpcl','left');
  ...
  {
    $this->db->select('cpcl.id_cpcl, cpcl.id_mst_riph, cpcl.no_pks, cpcl.tgl_rencana_tanam, cpcl.rencana_produksi, cpcl.tgl_awal_perjanjian, cpcl.tgl_akhir_perjanjian, cpcl.nama_pelaksana, cpcl.is_active, cpcl.is_verifikasi, if(cpcl.type_pelaksana=2, "Swakelola", "Kemitraan") as namapengelola, 
	wilayah_provinsi.nama as nama_prov, wilayah_kabupaten.nama as nama_kab, wilayah_kecamatan.nama as nama_kec, wilayah_desa.nama as nama_des, t1.jmlrealisasi AS jmlrealisasi,t2.jmlproduksi AS jmlproduksi');  
	$this->db->join('wilayah_provinsi', 'cpcl.provinsi = wilayah_provinsi.id');
	$this->db->join('wilayah_kabupaten', 'cpcl.kabupaten = wilayah_kabupaten.id');
	$this->db->join('wilayah_kecamatan', 'cpcl.kecamatan = wilayah_kecamatan.id');
  ...
  function get_all_marker_cpcl($idc)
  {
   ...
   function get_total_luas_cpcl_admin()
  {
