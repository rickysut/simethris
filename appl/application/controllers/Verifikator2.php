...
class Verifikator extends CI_Controller{
  ...
  function index()
  {
	...
	foreach($ajuVerif as $forPerusahaan){
		...
		$ia =0;$ii += 1;
		if($ii > 1){
			$DataTableKu .= ',{"data":{"Nama Perusahaan":"'.$myPerusahaan['nama_perusahaan'].'","Provinsi":"'.$myPerusahaan['nama_prov'].'","Kabupaten":"'.$myPerusahaan['nama_kab'].'","Kecamatan":"'.$myPerusahaan['nama_kec'].'","Desa":"'.$myPerusahaan['nama_des'].'"}';
		}else{
			$DataTableKu .= '{"data":{"Nama Perusahaan":"'.$myPerusahaan['nama_perusahaan'].'","Provinsi":"'.$myPerusahaan['nama_prov'].'","Kabupaten":"'.$myPerusahaan['nama_kab'].'","Kecamatan":"'.$myPerusahaan['nama_kec'].'","Desa":"'.$myPerusahaan['nama_des'].'"}';
		}
		...
		foreach($myriph as $forRIPH){
			...
			if(($jbrealisasi <= $jrealisasi) or ($jbproduksi <= $jproduksi)){
				$veriflunas = json_encode('<a href="'.base_url('verifikator/verifikasilunas/'.$forRIPH->id_mst_riph).'" data-toggle="tooltip" title="Verifikasi / Keterangan Lunas" class="btn btn-warning btn-fab btn-sm"><i class="icon-pencil-circle"></i></a>', JSON_HEX_TAG | JSON_HEX_AMP);					
			}else{
				$veriflunas = json_encode('<a href="'.base_url('verifikator/verifikasilunas/'.$forRIPH->id_mst_riph).'" data-toggle="tooltip" title="Verifikasi / Keterangan Lunas" class="btn btn-warning btn-fab btn-sm disabled" role="button" aria-disabled="true"><i class="icon-pencil-circle"></i></a>', JSON_HEX_TAG | JSON_HEX_AMP);
			}
			//$veriflunas = json_encode('<a href="'.base_url('verifikator/verifikasilunas/'.$forRIPH->id_mst_riph).'" data-toggle="tooltip" title="Verifikasi / Keterangan Lunas" class="btn btn-warning btn-fab btn-sm"><i class="icon-pencil-circle"></i></a>', JSON_HEX_TAG | JSON_HEX_AMP);					
			$DataTableKu .= ',"kids":[{"data":{"Nomor RIPH":"'.$forRIPH->nomor_riph.'","TGL Awal":"'.date('d-m-Y',strtotime($forRIPH->tgl_awal_riph)).'","TGL Akhir":"'.date('d-m-Y',strtotime($forRIPH->tgl_akhir_riph)).'","V.Import":"'.$forRIPH->volume.'","B. Tanam":"'.$forRIPH->kewajiban_tanam.'","B. Produksi":"'.$forRIPH->kewajiban_produksi.'","Luas Tanam":"'.$forRIPH->luas_tanam.'","Jml Produksi":"'.$forRIPH->jmlproduksi.'","Action":'.$veriflunas.'}';
			// -- daftar PKS
			...
			if($myPKS != null){
				...
				foreach($myPKS as $forPKS){
					$ib =0;
					if($forPKS->is_verifikasi == '1'){$is_verifikasi11 = json_encode('<button type="button" data-toggle="tooltip" title="PKS Sudah Diverifikasi" class="btn btn-success btn-fab btn-sm"><i class="icon-checkbox-marked-circle-outline"></i></button>', JSON_HEX_TAG | JSON_HEX_AMP);}
					elseif($forPKS->is_verifikasi == '2'){$is_verifikasi11 = json_encode('<button type="button" data-toggle="tooltip" title="PKS Sudah Diverifikasi Status Pending" class="btn btn-warning btn-fab btn-sm"><i class="icon-delta"></i></button>', JSON_HEX_TAG | JSON_HEX_AMP);}
					elseif($forPKS->is_verifikasi == '3'){$is_verifikasi11 = json_encode('<button type="button" data-toggle="tooltip" title="PKS Sudah Diverifikasi Status Ditolak" class="btn btn-danger btn-fab btn-sm"><i class="icon-cancel"></i></button>', JSON_HEX_TAG | JSON_HEX_AMP);}
					else{$is_verifikasi11 = json_encode('<button type="button" data-toggle="tooltip" title="PKS Belum Diverifikasi" class="btn btn-info btn-fab btn-sm"><i class="icon-close-circle"></i></button>', JSON_HEX_TAG | JSON_HEX_AMP);}
					
					$verifikasi11 = '<a href="'.base_url('verifikator/createverifikasi2/'.$forPKS->id_cpcl).'" data-toggle="tooltip" title="Verifikasi Langsung" class="btn btn-warning btn-fab btn-sm"><i class="icon-pencil-circle"></i></a>';
					$lihatPKS = '<a href="'.base_url('verifikator/lihatcpcl/'.$forPKS->id_cpcl).'" data-toggle="tooltip" title="Lihat PKS Untuk Diverifikasi" class="btn btn-warning btn-fab btn-sm"><i class="icon-eye"></i></a>';
					$ActionPKS = $lihatPKS .$verifikasi11;
					$ActionPKShtml = json_encode($ActionPKS, JSON_HEX_TAG | JSON_HEX_AMP);
					
					if($ia > 0){
						$DataTableKu .= ',{"data":{"ID PKS":"'.$forPKS->id_cpcl.'","Nama Pelaksana":"'.$forPKS->nama_pelaksana.'","Kabupaten":"'.$forPKS->nama_kab.'","Kecamatan":"'.$forPKS->nama_kec.'","Desa":"'.$forPKS->nama_des.'", "Status":'.$is_verifikasi11.', "Action":'.$ActionPKShtml.'},"kids":[';
					}
					else
					{
						$DataTableKu .= ',"kids":[{"data":{"ID PKS":"'.$forPKS->id_cpcl.'","Nama Pelaksana":"'.$forPKS->nama_pelaksana.'","Kabupaten":"'.$forPKS->nama_kab.'","Kecamatan":"'.$forPKS->nama_kec.'","Desa":"'.$forPKS->nama_des.'", "Status":'.$is_verifikasi11.', "Action":'.$ActionPKShtml.'},"kids":[';
					}
					$ia += 1;
					...
					if($myRealisasi != null){
						foreach ($myRealisasi as $forRealisasi){
							if($forRealisasi->is_verifikasi == '1'){$is_verifikasi_r = json_encode('<button type="button" data-toggle="tooltip" title="Realisasi Sudah Diverifikasi" class="btn btn-success btn-fab btn-sm"><i class="icon-checkbox-marked-circle-outline"></i></button>', JSON_HEX_TAG | JSON_HEX_AMP);}
							elseif($forRealisasi->is_verifikasi == '2'){$is_verifikasi_r = json_encode('<button type="button" data-toggle="tooltip" title="Realisasi Tanam Sudah Diverifikasi Status Pending" class="btn btn-warning btn-fab btn-sm"><i class="icon-delta"></i></button>', JSON_HEX_TAG | JSON_HEX_AMP);}
							elseif($forRealisasi->is_verifikasi == '3'){$is_verifikasi_r = json_encode('<button type="button" data-toggle="tooltip" title="Realisasi Tanam Sudah Diverifikasi Status Ditolak" class="btn btn-danger btn-fab btn-sm"><i class="icon-cancel"></i></button>', JSON_HEX_TAG | JSON_HEX_AMP);}
							else{$is_verifikasi_r = json_encode('<button type="button" data-toggle="tooltip" title="Realisasi Tanam Belum Diverifikasi" class="btn btn-info btn-fab btn-sm"><i class="icon-close-circle"></i></button>', JSON_HEX_TAG | JSON_HEX_AMP);}
							$clihat_r = '<a href="'.base_url('verifikator/lihatrealisasi/'.$forRealisasi->id_realisasi).'" data-toggle="tooltip" title="Lihat Untuk Diverifikasi" class="btn btn-warning btn-fab btn-sm"><i class="icon-eye"></i></a>';
							$cverifikasi_r = '<a href="'.base_url('verifikator/createverifikasi3/'.$forRealisasi->id_realisasi).'" data-toggle="tooltip" title="Verifikasi Langsung" class="btn btn-warning btn-fab btn-sm"><i class="icon-pencil-circle"></i></a>';
							...
							if($myProduksi != null){
								foreach($myProduksi as $forProduksi){
									...
									$lihat_p = '<a href="'.base_url('verifikator/lihatproduksi/'.$forProduksi->id_produksi).'" data-toggle="tooltip" title="Lihat Untuk Diverifikasi" class="btn btn-warning btn-fab btn-sm"><i class="icon-eye"></i></a>';
									$verifikasi_p = '<a href="'.base_url('verifikator/createverifikasi4/'.$forProduksi->id_produksi).'" data-toggle="tooltip" title="Verifikasi Langsung" class="btn btn-warning btn-fab btn-sm"><i class="icon-pencil-circle"></i></a>';
