...
class Dashboard extends CI_Controller {
	...
	public function index($thn)
	{
		...
		if ($this->session->userdata('usertype')=='3'){
			...
			$minggulalu2 = $minggulalu - 1;
			$minggulalu3 = $minggulalu2 - 1;
			$this->data['get_all_table_steps']  			= $this->Dashboard_model->get_all_cpcl_riph_dash($thn);
			$this->data['get_all_cpcl_admin']  				= $this->Dashboard_model->get_all_cpcl_adminc2();
			$this->data['get_jml_cpcl_verifikasi']  		= $this->Dashboard_model->get_jml_cpcl_verifikasi_admin($thn);
			$this->data['get_jml_cpcl_blverifikasi']  		= $this->Dashboard_model->get_jml_cpcl_blverifikasi_admin($thn);
			...
			$this->data['get_jml_real_tanam']  				= $this->Dashboard_model->get_all_realisasi_tanam_admin($thn);
			$this->data['get_jml_beban_tanam'] 				= $this->Dashboard_model->get_all_beban_realisasi_tanam_admin($thn);
			$this->data['get_jml_all_produksi'] 			= $this->Dashboard_model->get_all_jml_produksi_admin($thn);
			$this->data['get_jml_beban_produksi']			= $this->Dashboard_model->get_all_beban_produksi_admin($thn);
			...
			$this->data['get_jml_import']					= $this->Dashboard_model->get_volume_import($thn);
			$this->data['get_jml_verifikasi']				= $this->r_verifikasi->get_jml_r_verifikasi();
			$this->data['get_all_lo']  						= $this->Auth_model->get_all_users();
			$this->data['get_all_lo2']  					= $this->Auth_model->get_all_lo($thn);
			...
			$this->data['get_jml_riph_admin']				= $this->Riph_model->get_all_riph_admin_dasboard($thn);
			$this->data['get_jml_rencana_tanam'] 			= $this->Cpcl_model->get_total_luas_cpcl_admin($thn);
	...
	public function ajax_grafik_bulanan($bulan)
	{
		...
		$get_jml_riph_admin			= $this->Riph_model->get_all_riph_admin();
		$bebanTanamNasional = 0;
		$bebanProduksiNasional = 0;
		if (empty($get_jml_riph_admin->v_beban_tanam)) {
			$data['bebanTanamNasional'] = 0;
		} else {
			$data['bebanTanamNasional'] = $get_jml_riph_admin->v_beban_tanam;
		}
		if (empty($get_jml_riph_admin->v_beban_produksi)) {
			$data['bebanProduksiNasional'] = 0;
		} else {
			$data['bebanProduksiNasional'] = $get_jml_riph_admin->v_beban_produksi;
		}
		$chartRW = array();
		$jmlR = array();
		$jmlverif = 0;
		...
		foreach ($get_chart_verifikasi as $getVerif) {
			if (is_null($getVerif->jumlah_verif)) {
				...
				$data['jmlverif'] = $getVerif->jumlah_verif;
			}
		}
		foreach ($get_chart_unverifikasi as $getunVerif) {
			if (is_null($getunVerif->jumlah_unverif)) {
				...
				$data['jmlunverif'] = $getunVerif->jumlah_unverif;
		...
		foreach ($get_chart_verifikasi_w1 as $getVerifw1) {
			if (is_null($getVerifw1->jumlah_verif)) {
				...
				$data['jmlverifw1'] = $getVerifw1->jumlah_verif;
		...
		foreach ($get_chart_unverifikasi_w1 as $getunVerifw1) {
			if (is_null($getunVerifw1->jumlah_unverif)) {
				...
				$data['jmlunverifw1'] = $getunVerifw1->jumlah_unverif;
		...
		foreach ($get_chart_verifikasi_w2 as $getVerifw2) {
			if (is_null($getVerifw2->jumlah_verif)) {
				...
				$data['jmlverifw2'] = $getVerifw2->jumlah_verif;
		...
		foreach ($get_chart_unverifikasi_w2 as $getunVerifw2) {
			if (is_null($getunVerifw2->jumlah_unverif)) {
				...
				$data['jmlunverifw2'] = $getunVerifw2->jumlah_unverif;
		...
		foreach ($get_chart_verifikasi_w3 as $getVerifw3) {
			if (is_null($getVerifw3->jumlah_verif)) {
				...
				$data['jmlverifw3'] = $getVerifw3->jumlah_verif;
		...
		foreach ($get_chart_unverifikasi_w3 as $getunVerifw3) {
			if (is_null($getunVerifw3->jumlah_unverif)) {
				...
				$data['jmlunverifw3'] = $getunVerifw3->jumlah_unverif;
				$jmlunverifw3 = $getunVerifw3->jumlah_unverif;
			}
		}
		$data['jmlchartPKSw3'] = $jmlverifw3 + $jmlunverifw3;
		$data['jmlchartPKSw2'] = $jmlverifw2 + $jmlunverifw2;
		$data['jmlchartPKSw1'] = $jmlverifw1 + $jmlunverifw1;
		$data['jmlchartPKS'] = $jmlverif + $jmlunverif;
		foreach ($get_chart_beban as $getchartbeban) {
			if (is_null($getchartbeban->bebantanam)) {
				$data['jmlbrealisasi'] = 0;
				$jmlbrealisasi = 0;
			} else {
				$data['jmlbrealisasi'] = $getchartbeban->bebantanam;
			...
			if (is_null($getchartbeban->bebanproduksi)) {
				...
				$data['jmlbproduksi'] = $getchartbeban->bebanproduksi;
		...
		foreach ($get_chart_beban_w1 as $getchartbebanw1) {
			if (is_null($getchartbebanw1->bebantanam)) {
				$data['jmlbrealisasiw1'] = 0;
				$jmlbrealisasiw1 = 0;
			} else {
				$data['jmlbrealisasiw1'] = $getchartbebanw1->bebantanam;
				$jmlbrealisasiw1 = $getchartbebanw1->bebantanam;
			}
			if (is_null($getchartbebanw1->bebanproduksi)) {
				$data['jmlbproduksiw1'] = 0;
				$jmlbproduksiw1 = 0;
			} else {
				$data['jmlbproduksiw1'] = $getchartbebanw1->bebanproduksi;
		...
		foreach ($get_chart_beban_w2 as $getchartbebanw2) {
			if (is_null($getchartbebanw2->bebantanam)) {
				$data['jmlbrealisasiw2'] = 0;
				$jmlbrealisasiw2 = 0;
			} else {
				$data['jmlbrealisasiw2'] = $getchartbebanw2->bebantanam;
				$jmlbrealisasiw2 = $getchartbebanw2->bebantanam;
			}
			if (is_null($getchartbebanw2->bebanproduksi)) {
				$data['jmlbproduksiw2'] = 0;
				$jmlbproduksiw2 = 0;
			} else {
				$data['jmlbproduksiw2'] = $getchartbebanw2->bebanproduksi;
		...
		foreach ($get_chart_beban_w3 as $getchartbebanw3) {
			if (is_null($getchartbebanw3->bebantanam)) {
				$data['jmlbrealisasiw3'] = 0;
				$jmlbrealisasiw3 = 0;
			} else {
				$data['jmlbrealisasiw3'] = $getchartbebanw3->bebantanam;
				$jmlbrealisasiw3 = $getchartbebanw3->bebantanam;
			}
			if (is_null($getchartbebanw3->bebanproduksi)) {
				$data['jmlbproduksiw3'] = 0;
				$jmlbproduksiw3 = 0;
			} else {
				$data['jmlbproduksiw3'] = $getchartbebanw3->bebanproduksi;
		...
		foreach ($get_chart_jmlrealisasi as $getchartrealisasi) {
			if (is_null($getchartrealisasi->jumlah_realisasi)) {
				$data['jmlchartrealisasi'] = 0;
				$jmlchartrealisasi = 0;
			} else {
				$data['jmlchartrealisasi'] = $getchartrealisasi->jumlah_realisasi;
		...
		foreach ($get_chart_jmlrealisasi_w1 as $getchartrealisasiw1) {
			if (is_null($getchartrealisasiw1->jumlah_realisasi)) {
				$data['jmlchartrealisasiw1'] = 0;
				$jmlchartrealisasiw1 = 0;
			} else {
				$data['jmlchartrealisasiw1'] = $getchartrealisasiw1->jumlah_realisasi;
		...
		foreach ($get_chart_jmlrealisasi_w2 as $getchartrealisasiw2) {
			if (is_null($getchartrealisasiw2->jumlah_realisasi)) {
				$data['jmlchartrealisasiw2'] = 0;
				$jmlchartrealisasiw2 = 0;
			} else {
				$data['jmlchartrealisasiw2'] = $getchartrealisasiw2->jumlah_realisasi;
		...
		foreach ($get_chart_jmlrealisasi_w3 as $getchartrealisasiw3) {
			if (is_null($getchartrealisasiw3->jumlah_realisasi)) {
				$data['jmlchartrealisasiw3'] = 0;
				$jmlchartrealisasiw3 = 0;
			} else {
				$data['jmlchartrealisasiw3'] = $getchartrealisasiw3->jumlah_realisasi;
		...
		foreach ($get_chart_jmlproduksi as $getchartproduksi) {
			if (is_null($getchartproduksi->jumlah_produksi)) {
				$data['jmlchartproduksi'] = 0;
				$jmlchartproduksi = 0;
			} else {
				$data['jmlchartproduksi'] = $getchartproduksi->jumlah_produksi;
		...
		foreach ($get_chart_jmlproduksi_w1 as $getchartproduksiw1) {
			if (is_null($getchartproduksiw1->jumlah_produksi)) {
				$data['jmlchartproduksiw1'] = 0;
				$jmlchartproduksiw1 = 0;
			} else {
				$data['jmlchartproduksiw1'] = $getchartproduksiw1->jumlah_produksi;
		...
		foreach ($get_chart_jmlproduksi_w2 as $getchartproduksiw2) {
			if (is_null($getchartproduksiw2->jumlah_produksi)) {
				$data['jmlchartproduksiw2'] = 0;
				$jmlchartproduksiw2 = 0;
			} else {
				$data['jmlchartproduksiw2'] = $getchartproduksiw2->jumlah_produksi;
		...
		foreach ($get_chart_jmlproduksi_w3 as $getchartproduksiw3) {
			if (is_null($getchartproduksiw3->jumlah_produksi)) {
				$data['jmlchartproduksiw3'] = 0;
				$jmlchartproduksiw3 = 0;
			} else {
				$data['jmlchartproduksiw3'] = $getchartproduksiw3->jumlah_produksi;
