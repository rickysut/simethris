...
class Produksi extends CI_Controller{
  ...
  function create($idr)
  {
    ...
    if($this->data['user'])
    {
      ...
      $this->data['action']     = 'produksi/create_action/'.$idr;
      $this->data['get_all_combobox_provinsi'] = $this->m_wilayah->get_all_provinsi2();
  ...
  function create_action($id)
  {
    ...
    else
    {
		...
		if($_FILES['kegiatan4']['error'] <> 4)
		{
			...
			{
			  $kegiatan4 = $this->upload->data();

			  $thumbnail                  = $config['file_name'];

			  $config['image_library']    = 'gd2';
			  $config['source_image']     = './uploads/produksi/'.$kegiatan4['file_name'].'';
			  $config['create_thumb']     = TRUE;
			  $config['maintain_ratio']   = TRUE;
			  $config['width']            = 250;
			  $config['height']           = 250;
