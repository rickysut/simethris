...
class Auth_model extends CI_Model{
  ...
  function get_all_lo($thn)
  {
    $this->db->select('count(id_users) as jml_lo');
    $this->db->where('is_delete', '0');
    $this->db->where('is_active', '1');
	  $this->db->where('usertype', '3');
    if ($thn && ($thn!='0') && ($thn!='index')) {
      $this->db->where('DATE_FORMAT(created_at, "%Y") =', $thn); 
    }
  ...
  {
     return $this->db->get($this->viewlog)->result();
  }
