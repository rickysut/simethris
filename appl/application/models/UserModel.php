...
class UserModel extends CI_Model {
	...
	{  
 		$this->db->select('id_users,name,photo,is_online,DATE_FORMAT(last_login, "%d%m%Y") as last_login');
		$this->db->from($this->User);
		$where = "(usertype=2 OR usertype=1) AND is_active=1";
		$this->db->where($where);
	...
	{  
 		$this->db->select('id_users,name,photo,is_online,DATE_FORMAT(last_login, "%d%m%Y") as last_login');
		$this->db->from($this->User);
		$where = "(usertype=3) AND is_active=1";
		$this->db->where($where);
   		$query = $this->db->get();
 		$r=$query->result_array();
		return $r;
