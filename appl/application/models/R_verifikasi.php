...
class R_verifikasi extends CI_Model{
  ...
  function get_all_cpcl_by_user($id)
  {
    $this->db->select('id_users,id_cpcl,no_pks,nama_pelaksana, tgl_rencana_tanam, luas_rencana_tanam, is_verifikasi, wilayah_provinsi.nama as nama_prov,wilayah_kabupaten.nama as nama_kab,
  ...
  function get_all_r_verifikasi_by_jenis($idj)
  {
    $this->db->where('is_delete', '0');
	$this->db->where('jenis_verifikasi', $idj);
	//$this->db->order_by('nama_perusahaan', $this->order);
