  ...
  function index()
  {
	...
	$myPKids=array();
		foreach($ajuVerif as $forPerusahaan){
			...
			// tingkat 1 perusahaan
			$myP = array("data" => ["Nama Perusahaan"=>$myPerusahaan['nama_perusahaan'],"Provinsi"=>$myPerusahaan['nama_prov'],"Kabupaten"=>$myPerusahaan['nama_kab'],"Kecamatan"=>$myPerusahaan['nama_kec'],"Desa"=>$myPerusahaan['nama_des']],);
			$myPKS = $this->r_verifikasi->get_all_cpcl_by_user($forPerusahaan->id_users);
			//array_push($mydatatable, $myP);
			if(is_null($myPKS)){
				$myP = array("data" => array("Nama Perusahaan"=>$myPerusahaan['nama_perusahaan'],"Provinsi"=>$myPerusahaan['nama_prov'],"Kabupaten"=>$myPerusahaan['nama_kab'],"Kecamatan"=>$myPerusahaan['nama_kec'],"Desa"=>$myPerusahaan['nama_des']),"kids"=>[]);
				array_push($mydatatable,$myP);
			}
			else
			{
				...
				foreach($myPKS as $forPKS){
					if($forPKS->is_verifikasi == '1'){$is_verifikasi11 = '<button type="button" class="btn btn-xs btn-success">verified <span class="badge">V</span></button>';}
					else{$is_verifikasi11 = '<button type="button" class="btn btn-xs btn-danger">Unverified <span class="badge">X</span></button>';}
					$verifikasi11 = '<a href="'.base_url('verifikator/createverifikasi1/'.$forPKS->id_cpcl).'" data-toggle="tooltip" title="Verifikasi PKS" class="btn btn-xs btn-warning"><i class="fa fa-pencil"></i></a>';
					...
					$myP["kids"]=[array("data" => array("ID PKS"=>$forPKS->id_cpcl,"Nama Pelaksana"=>$forPKS->nama_pelaksana,"Kabupaten"=>$forPKS->nama_kab,"Kecamatan"=>$forPKS->nama_kec,"Desa"=>$forPKS->nama_des, "Status"=>$is_verifikasi11, "Action"=>$verifikasi11)),];
					$myidcpcl = $forPKS->id_cpcl;
					$myRealisasi = $this->r_verifikasi->get_all_realisasi_by_idcpcl($forPKS->id_cpcl);
					if(is_null($myRealisasi)){
						$myP["kids"]=[array("data" => array("ID PKS"=>$forPKS->id_cpcl,"Nama Pelaksana"=>$forPKS->nama_pelaksana,"Kabupaten"=>$forPKS->nama_kab,"Kecamatan"=>$forPKS->nama_kec,"Desa"=>$forPKS->nama_des, "Status"=>$is_verifikasi11, "Action"=>$lihatPKS .$verifikasi11),"kids"=>[])];
						array_push($mydatatable,$myP);
					}
					else
					{
						foreach($myRealisasi as $forRealisasi){
							if($forRealisasi->is_verifikasi == '1'){$is_verifikasi_r = '<button type="button" class="btn btn-xs btn-success">verified <span class="badge">V</span></button>';}
							else{$is_verifikasi_r = '<button type="button" class="btn btn-xs btn-danger">Unverified <span class="badge">X</span></button>';}
							// action
							$lihat_r = '<a href="'.base_url('verifikator/lihatrealisasi/'.$forRealisasi->id_realisasi).'" data-toggle="tooltip" title="Lihat Realisasi" class="btn btn-xs btn-success"><i class="fa fa-eye"></i></a>';
							$verifikasi_r = '<a href="'.base_url('verifikator/createverifikasi2/'.$forRealisasi->id_realisasi).'" data-toggle="tooltip" title="Verifikasi Realisasi Tanam" class="btn btn-xs btn-warning"><i class="fa fa-pencil"></i></a>';

							$myP["kids"]=[array("data" => ["ID PKS"=>$forPKS->id_cpcl,"Nama Pelaksana"=>$forPKS->nama_pelaksana,"Kabupaten"=>$forPKS->nama_kab,"Kecamatan"=>$forPKS->nama_kec,"Desa"=>$forPKS->nama_des, "Status"=>$is_verifikasi11, "Action"=>$lihatPKS .$verifikasi11],
							"kids"=>array("data" => ["ID Realisasi"=>$forRealisasi->id_realisasi,"Nama Pelaksana"=>$forRealisasi->nama_pelaksana,"TGL Realisasi"=>$forRealisasi->tgl_realisasi_tanam,"Luas"=>$forRealisasi->luas_realisasi_tanam,"Status"=>$is_verifikasi_r, "Action"=>$lihat_r . $verifikasi_r]))];
							//array_push($mydatatable, $myP);
							$myProduksi=$this->r_verifikasi->get_all_produksi_by_idrealisasi($forRealisasi->id_realisasi);
							if(is_null($myProduksi)){
								$myP["kids"]=[array("data" => ["ID PKS"=>$forPKS->id_cpcl,"Nama Pelaksana"=>$forPKS->nama_pelaksana,"Kabupaten"=>$forPKS->nama_kab,"Kecamatan"=>$forPKS->nama_kec,"Desa"=>$forPKS->nama_des, "Status"=>$is_verifikasi11, "Action"=>$lihatPKS .$verifikasi11],
								"kids"=>array("data" => ["ID Realisasi"=>$forRealisasi->id_realisasi,"Nama Pelaksana"=>$forRealisasi->nama_pelaksana,"TGL Realisasi"=>$forRealisasi->tgl_realisasi_tanam,"Luas"=>$forRealisasi->luas_realisasi_tanam,"Status"=>$is_verifikasi_r, "Action"=>$lihat_r . $verifikasi_r],"kids"=>[]))];
								array_push($mydatatable,$myP);
							}else{
								foreach($myProduksi as $forProduksi){
									...
									$myP["kids"]=[array("data" => array("ID PKS"=>$forPKS->id_cpcl,"Nama Pelaksana"=>$forPKS->nama_pelaksana,"Kabupaten"=>$forPKS->nama_kab,"Kecamatan"=>$forPKS->nama_kec,"Desa"=>$forPKS->nama_des, "Status"=>$is_verifikasi11, "Action"=>$lihatPKS .$verifikasi11),
									"kids"=>[array("data" => array("ID Realisasi"=>$forRealisasi->id_realisasi,"Nama Pelaksana"=>$forRealisasi->nama_pelaksana,"TGL Realisasi"=>$forRealisasi->tgl_realisasi_tanam,"Luas"=>$forRealisasi->luas_realisasi_tanam,"Status"=>$is_verifikasi_r, "Action"=>$lihat_r . $verifikasi_r),
									"kids"=>[array("data" => array("ID Produksi"=>$forProduksi->id_produksi,"Nama Pelaksana"=>$forProduksi->nama_pelaksana,"TGL Produksi"=>$forProduksi->tgl_produksi,"Jumlah"=>$forProduksi->jml_produksi,"Status"=>$is_verifikasi_p, "Action"=>$lihat_p . $verifikasi_p),
									"kids"=>[])])])];
									//$myP2["kids"]=[array("data" => array("ID Realisasi"=>$forRealisasi->id_realisasi,"Nama Pelaksana"=>$forRealisasi->nama_pelaksana,"TGL Realisasi"=>$forRealisasi->tgl_realisasi_tanam,"Luas"=>$forRealisasi->luas_realisasi_tanam),
									//"kids"=>[array("data" => array("ID Produksi"=>$forProduksi->id_produksi,"Nama Pelaksana"=>$forProduksi->nama_pelaksana,"TGL Produksi"=>$forProduksi->tgl_produksi,"Jumlah"=>$forProduksi->jml_produksi),
									//"kids"=>[])])];
									array_push($mydatatable,$myP);
								}
							}
	...
	$this->data['data_for_table'] = $mydatatable;
