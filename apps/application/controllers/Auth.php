...
class Auth extends CI_Controller{
  ...
  function update_profile_action()
  {
    ...
    else
    {
	  ...
	  {
		$nmfileriph = strtolower(url_title($this->input->post('username'))).date('YmdHis');
    ...
    if($this->form_validation->run() === TRUE)
    {
      ...
      else
      {
		...
		if(empty($row->logo_perusahaan)){
            $logoper = base_url().'/assets/images/logo_dirjen.png';
            }else{
            $logoper = base_url('/uploads/company/').$row->logo_perusahaan;
        }
        $session = array(
  ...
  function registration_action()
  {
    ...
    $this->form_validation->set_rules('phone_number', 'Phone No', 'trim|is_numeric');
	$this->form_validation->set_rules('kode_pos', 'Kode POS', 'trim|is_numeric');
	if (empty($_FILES['photo']['name'])){$this->form_validation->set_rules('photo', 'Photo', 'required');}
	if (empty($_FILES['file_sk']['name'])){$this->form_validation->set_rules('file_sk', 'Scan File SK', 'required');}
	if (empty($_FILES['file_ktp']['name'])){$this->form_validation->set_rules('file_ktp', 'Scan KTP', 'required');}
	if (empty($_FILES['logo_perusahaan']['name'])){$this->form_validation->set_rules('logo_perusahaan', 'Scan Logo Perusahaan', 'required');}
    ...
    else
    {
	  ...
	  if($_FILES['photo']['error'] <> 4)
      {
        ...
        $config['upload_path']      = './uploads/user/';
        $config['allowed_types']    = 'jpeg|jpg|png|txt|pdf|doc|docx|xls|xlsx|pptx|rtf';
        $config['max_size']         = 2048; // 2Mb
        $config['file_name']        = $nmfile;
        $this->load->library('upload', $config);
		$this->upload->initialize($config);
      ...
      if($_FILES['file_sk']['error'] <> 4)
	  {
        ...
        $config1['upload_path']      = './uploads/file_sk/';
        $config1['allowed_types']    = 'jpeg|jpg|png|txt|pdf|doc|docx|xls|xlsx|pptx|rtf';
        $config1['max_size']         = 2048; // 2Mb
        $config1['file_name']       = $nmfilesk;
	  ...
	  if($_FILES['file_ktp']['error'] <> 4)
	  {
        ...
        $config2['upload_path']      = './uploads/file_ktp/';
        $config2['allowed_types']    = 'jpeg|jpg|png|txt|pdf|doc|docx|xls|xlsx|pptx|rtf';
        $config2['max_size']         = 2048; // 2Mb
        $config2['file_name']       = $nmfilektp;
	  ...
	  if($_FILES['logo_perusahaan']['error'] <> 4)
	  {
		...
		// send email
          ...
          $mailContent = "
            ...
            <p>Untuk Pengaktifan user, silahkan tunggu sampai data yang anda kirimkan diverifikasi oleh Administrator.</p>
			<p>Terima Kasih.</p>
			<p>simeTHRIS</p>
          ";
